...
const mJe = (t, e, r) =>
  e in t ? pJe(t, e, { enumerable: !0, configurable: !0, writable: !0, value: r }) : (t[e] = r);
...
const Cz = jt({ blockNumber: Et(), blockTimestamp: Et() });
...
class lwt extends $f {
  ...
  write(e, r, n) {
    ...
    for (const a of i) {
      const s = [...n, a];
      this.key.write(e, a, s), this.value.write(e, r[a], [...n, a]);
    }
  }
  read(e, r) {
    const n = {},
      i = e.readLong();
    for (let a = 0; a < i; a++) {
      const s = this.key.read(e, [...r, `key_${a}`]),
        o = this.value.read(e, [...r, s]);
      n[s] = o;
    }
    return n;
  }
...
const AU = (t, e) => new lwt(t, e);
const ts = jt({ m5: Et(), h1: Et(), h6: Et(), h24: Et() });
const eB = jt({ buys: Et(), sells: Et() });
const Sm = jt({ address: He(), name: He(), symbol: He() });
const JM = Sm.extend({ decimals: Et().optional() });
const kz = JM.extend({ totalSupply: He().optional() });
const Z1 = jt({ m5: eB, h1: eB, h6: eB, h24: eB });
const rs = jt({ m5: Et().optional(), h1: Et().optional(), h6: Et().optional(), h24: Et().optional() });
const Q1 = jt({ m5: Et().optional(), h1: Et().optional(), h6: Et().optional(), h24: Et().optional() });
const Y1 = jt({ usd: Et(), base: Et(), quote: Et() });
const BDe = jt({
  eti: zi(),
  header: zi().optional(),
  website: zi().optional(),
  twitter: zi().optional(),
  discord: zi().optional(),
  linkCount: Et().optional(),
  imgKey: He().optional(),
  nsfw: zi().optional(),
});
const ODe = jt({
  headerId: He().optional(),
  iconId: He(),
  description: He().optional(),
  links: Kn(
    jt({ type: U1(["telegram", "twitter", "discord", "facebook", "tiktok"]).optional(), url: He() })
  ).optional(),
});
const Iz = jt({ active: Et() });

const c9 = jt({
  chainId: He(),
  dexId: He(),
  labels: Kn(He()).optional(),
  pairAddress: He(),
  baseToken: kz,
  quoteToken: JM,
  quoteTokenSymbol: He(),
  price: He(),
  priceUsd: He().optional(),
  txns: Z1,
  buyers: ts.optional(),
  sellers: ts.optional(),
  makers: ts.optional(),
  volume: rs,
  volumeBuy: rs.optional(),
  volumeSell: rs.optional(),
  priceChange: Q1,
  liquidity: Y1.optional(),
  marketCap: Et().optional(),
  fdv: Et().optional(),
  pairCreatedAt: Et().optional(),
  profile: BDe.optional(),
  cmsProfile: ODe.optional(),
  isBoostable: zi(),
  boosts: Iz.optional(),
});
...
const UDe = jt({ progress: Et(), creator: He(), migrationDex: He(), meta: AU(He(), He()).optional() });

const $Ct = jt({
  c: Bi("a"),
  a: He(),
  cgi: He().optional().nullable(),
  moonshot: nC.optional(),
  launchpad: UDe.optional(),
});

const pue = c9.merge($Ct);

const W1 = jt({ c: xce().optional() });
const jCt = c9.merge(W1);

const q1 = jt({ c: Bi("u") });
const zCt = c9.merge(q1);

const G1 = jt({ c: Bi("b"), pn: He() });
const WCt = c9.merge(G1);

const K1 = jt({ c: Bi("o"), cgi: He().optional() });
const GCt = c9.merge(K1);

const rB = jt({ txns: Et(), volumeUsd: Et() });
const Pz = jt({ m5: rB, h1: rB, h6: rB, h24: rB });
const mue = Fh([pue, jCt, zCt, WCt, GCt]);

export const C7e = jt({
  schemaVersion: Bi("1.3.0"),
  type: Bi("pairs"),
  stats: Pz,
  pairs: Kn(mue),
  pairsCount: Et().optional(),
});

export const T7e = jt({ schemaVersion: Bi("1.3.0"), type: Bi("latestBlock"), latestBlock: Cz.nullable() });
